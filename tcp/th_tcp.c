...
extern long sysctl_tcp_mem[3] __read_mostly;
extern int sysctl_tcp_wmem[3] __read_mostly;
extern int sysctl_tcp_rmem[3] __read_mostly;

//EXPORT_SYMBOL(sysctl_tcp_mem);
//EXPORT_SYMBOL(sysctl_tcp_rmem);
//EXPORT_SYMBOL(sysctl_tcp_wmem);

extern atomic_long_t tcp_memory_allocated;	/* Current allocated memory. */
 ...
 */
extern unsigned int tcp_poll(struct file *file, struct socket *sock, poll_table *wait);
/*{
	unsigned int mask;
 ...
 *
 **/
extern ssize_t tcp_splice_read(struct socket *sock, loff_t *ppos,
...
extern int tcp_setsockopt(struct sock *sk, int level, int optname, char __user *optval,
		   unsigned int optlen);
/*{
	const struct inet_connection_sock *icsk = inet_csk(sk);
...
extern void tcp_get_info(const struct sock *sk, struct tcp_info *info);
/*{
	const struct tcp_sock *tp = tcp_sk(sk);
	const struct inet_connection_sock *icsk = inet_csk(sk);
...
extern int tcp_getsockopt(struct sock *sk, int level, int optname, char __user *optval,
		   int __user *optlen);
/*{
	struct inet_connection_sock *icsk = inet_csk(sk);
...
extern int tcp_md5_hash_skb_data(struct tcp_md5sig_pool *hp,
...
extern int tcp_md5_hash_key(struct tcp_md5sig_pool *hp, const struct tcp_md5sig_key *key);
/*{
	struct scatterlist sg;
